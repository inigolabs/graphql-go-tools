...
func (f *Fetcher) Fetch(ctx *Context, fetch *SingleFetch, preparedInput *fastbuffer.FastBuffer, buf *BufPair) (err error) {
	...
	if !f.EnableSingleFlightLoader || fetch.DisallowSingleFlight {
		...
		if ctx.afterFetchHook != nil {
			if buf.HasData() {
				ctx.afterFetchHook.OnData(f.hookCtx(ctx), dataBuf.Bytes(), buf.Data.Bytes(), ctx.path(), false)
			}
			if buf.HasErrors() {
				ctx.afterFetchHook.OnError(f.hookCtx(ctx), dataBuf.Bytes(), buf.Errors.Bytes(), ctx.path(), false)
			}
		}
		return
	}
	...
	if ok {
		...
		if inflight.bufPair.HasData() {
			if ctx.afterFetchHook != nil {
				ctx.afterFetchHook.OnData(f.hookCtx(ctx), inflight.bufPair.Data.Bytes(), inflight.bufPair.Data.Bytes(), ctx.path(), true)
			}
			buf.Data.WriteBytes(inflight.bufPair.Data.Bytes())
		}
		if inflight.bufPair.HasErrors() {
			if ctx.afterFetchHook != nil {
				ctx.afterFetchHook.OnError(f.hookCtx(ctx), inflight.bufPair.Errors.Bytes(), inflight.bufPair.Errors.Bytes(), ctx.path(), true)
			}
	...
	if inflight.bufPair.HasData() {
		if ctx.afterFetchHook != nil {
			ctx.afterFetchHook.OnData(f.hookCtx(ctx), dataBuf.Bytes(), inflight.bufPair.Data.Bytes(), ctx.path(), false)
		}
		buf.Data.WriteBytes(inflight.bufPair.Data.Bytes())
	}
	...
	if inflight.bufPair.HasErrors() {
		if ctx.afterFetchHook != nil {
			ctx.afterFetchHook.OnError(f.hookCtx(ctx), dataBuf.Bytes(), inflight.bufPair.Errors.Bytes(), ctx.path(), true)
		}
		buf.Errors.WriteBytes(inflight.bufPair.Errors.Bytes())
	}
